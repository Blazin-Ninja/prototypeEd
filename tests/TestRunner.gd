...
func _test_level_up() -> int:
	...
	# Debug set-level applies bonuses (not just the level field)
	var debug_pup := CreatureFactory.create_from_template("ember_pup", {"is_player": true})
	LevelSystem.ensure_fields(debug_pup)
	var dbg_atk := int(debug_pup.get("stats", {}).get("attack", 0))
	var dbg_hp := int(debug_pup.get("max_hp", 0))
	var dbg := LevelSystem.debug_set_level(debug_pup, 5)
	f += _ok("debug_set_level reaches 5", int(debug_pup.get("level", 0)) == 5 and int(dbg.get("levels_gained", 0)) == 4)
	f += _ok("debug_set_level raises attack", int(debug_pup.get("stats", {}).get("attack", 0)) > dbg_atk)
	f += _ok("debug_set_level raises max hp", int(debug_pup.get("max_hp", 0)) > dbg_hp)
	var dbg_cap := LevelSystem.debug_set_level(debug_pup, 99)
	f += _ok("debug_set_level caps at 20", int(debug_pup.get("level", 0)) == LevelSystem.MAX_LEVEL and int(dbg_cap.get("level", 0)) == LevelSystem.MAX_LEVEL)
	var no_op := LevelSystem.debug_set_level(debug_pup, 10)
	f += _ok("debug_set_level increase-only", int(no_op.get("levels_gained", -1)) == 0 and int(debug_pup.get("level", 0)) == LevelSystem.MAX_LEVEL)
	var grant := CreatureFactory.create_from_template("ember_pup", {"is_player": true})
	LevelSystem.ensure_fields(grant)
	var granted := LevelSystem.debug_grant_levels(grant, 3)
	f += _ok("debug_grant_levels +3", int(grant.get("level", 0)) == 4 and int(granted.get("levels_gained", 0)) == 3)
	# Debug path triggers basilisk evolution at Lv 8
	var dbg_basilisk := CreatureFactory.create_from_template("basilisk", {"is_player": true})
	LevelSystem.ensure_fields(dbg_basilisk)
	var dbg_evo := LevelSystem.debug_set_level(dbg_basilisk, 8)
	f += _ok("debug_set_level evolves basilisk", bool(dbg_evo.get("evolved", false)))
	f += _ok("debug evo becomes dread basilisk", str(dbg_basilisk.get("template_id", "")) == "dread_basilisk")
	f += _ok("debug evo keeps level 8", int(dbg_basilisk.get("level", 0)) == 8)
	return f
